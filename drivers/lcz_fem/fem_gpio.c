...
#define FEM_PDN_PIN DT_GPIO_PIN(FEM_NODE, pdn_gpios)
#else
...
static const struct gpio_dt_spec fem_spi_csn = GPIO_DT_SPEC_GET(FEM_GPIO_NODE, spi_csn_gpios);
static const struct gpio_dt_spec fem_spi_miso = GPIO_DT_SPEC_GET(FEM_GPIO_NODE, spi_miso_gpios);

#define PDN_PIN FEM_PDN_PIN
#define CSN_PIN DT_GPIO_PIN(FEM_GPIO_NODE, spi_csn_gpios)
#define ANT_PIN DT_GPIO_PIN(FEM_GPIO_NODE, ant_sel_gpios)
...
static int setup_follower(nrfx_gpiote_pin_t follower)
{
	...
	LOG_WRN("channel: in: %u out: %u ppi: %u", in_channel, out_channel, ppi_channel);

	uint8_t test_channel;

	test_channel = 0xFF;
	err = nrfx_gpiote_channel_get(PDN_PIN, &test_channel);
	LOG_INF("pdn status: %d ch: %u", NRFX_SUCCESS - err, test_channel);

	test_channel = 0xFF;
	err = nrfx_gpiote_channel_get(follower, &test_channel);
	LOG_INF("follower status: %d ch: %u", NRFX_SUCCESS - err, test_channel);
...
static int fem_gpio_init(const struct device *dev)
{
	...
	/* remove this or output config above when PPI is working */
	configure_output(&fem_spi_csn, 1); /* set active low signal to '0' */
	...
	LOG_INF("interrupt number %u", nrfx_get_irq_number(NRF_GPIOTE));
